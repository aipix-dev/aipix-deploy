...
export CONTROLLER_ONVIF_EXTERNAL_PORT=$(kubectl -n ${TRAEFIK_NAMESPACE} get service traefik -o jsonpath='{.spec.ports[?(@.name=="contr-onvif")].port}')
...
sed -i "s@APP_URL=.*@APP_URL=https://${VMS_DOMAIN}@g" ../vms-backend/environments/.env
