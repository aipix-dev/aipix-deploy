...
echo -e "\033[32mRUN CONTROLLER MIGRATIONS\033[0m"

###Run migrations
#Controller
kubectl -n ${NS_VMS} exec deployment.apps/controller-api -- ./scripts/update.sh

OUTPUT_PATH="../migrations/$(date +%Y-%m-%d)"
mkdir -p ${OUTPUT_PATH}

kubectl -n ${NS_VMS} exec deployment.apps/controller-api -- php artisan temporary:migrate-data-from-vms-to-controller --critical=true --cameras=true > ${OUTPUT_PATH}/cameras.log
kubectl -n ${NS_VMS} exec deployment.apps/controller-api -- php artisan temporary:migrate-data-from-vms-to-controller --critical=true --tags=true > ${OUTPUT_PATH}/tags.log
kubectl -n ${NS_VMS} exec deployment.apps/controller-api -- php artisan temporary:migrate-data-from-vms-to-controller --critical=true --vendors=true > ${OUTPUT_PATH}/vendors.log
kubectl -n ${NS_VMS} exec deployment.apps/controller-api -- php artisan temporary:migrate-data-from-vms-to-controller --critical=true --config_templates=true > ${OUTPUT_PATH}/config_templates.log
kubectl -n ${NS_VMS} exec deployment.apps/controller-api -- php artisan temporary:migrate-data-from-vms-to-controller --critical=true --camera_attributes=true > ${OUTPUT_PATH}/camera_attributes.log
kubectl -n ${NS_VMS} exec deployment.apps/controller-api -- php artisan temporary:migrate-data-from-vms-to-controller --critical=true --stream_attributes=true > ${OUTPUT_PATH}/stream_attributes.log
